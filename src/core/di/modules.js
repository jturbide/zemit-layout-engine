/**
 * Zemit Module Initialization
 * @author: <contact@dannycoulombe.com>
 * Creation date: 2018-09-24
 * 
 * Initialize all the modules
 */
(function() {
	
	Zemit.modules = {};
	
	var moduleList = [];
	var extendModuleList = [];
	Zemit.module = (name, configs = {}, args = {}) => {
		
		configs.priority = configs.priority === undefined ? 0 : configs.priority;
		configs.dependencies = configs.dependencies === undefined ? [] : configs.dependencies;
		configs.group = configs.group === undefined ? 'misc' : configs.group;
		configs.extensions = configs.extensions === undefined ? [] : configs.extensions;
		
		moduleList.push([name, configs, args]);
	};
	Zemit.extendModule = (name, configs = {}, args = {}) => {
		
		configs.priority = configs.priority === undefined ? 0 : configs.priority;
		configs.dependencies = configs.dependencies === undefined ? [] : configs.dependencies;
		configs.group = configs.group === undefined ? 'misc' : configs.group;
		configs.extensions = configs.extensions === undefined ? [] : configs.extensions;
		
		extendModuleList.push([name, configs, args]);
	};
	
	Zemit.app.config(['$ocLazyLoadProvider', function($ocLazyLoadProvider) {
		
		$ocLazyLoadProvider.config({
			// debug: Zemit.version === 'dev',
			events: true
		});
	}]);
	
	Zemit.app.run(['$hook', '$modules', '$util', '$injector', '$debug', '$i18n', '$sidebar', '$session', ($hook, $modules, $util, $injector, $debug, $i18n, $sidebar, $session) => {
		
		$hook.add('onReady', () => $debug.init('module', $i18n.get('modules.debugTitle')));
		
		$hook.add('onAllModulesLoaded', async function() {
			
			// Update module function to execute directly onAllModulesLoaded
			let modulesConfigured = [];
			Zemit.module = async function(name, configs = {}, args = {}) {
				
				let isExtension = modulesConfigured.indexOf(name) !== -1;
				
				let callbackParams = args.slice(0, args.length - 1);
				let callbackFunc = args.slice(args.length - 1, args.length)[0];
				let injectors = [];
				callbackParams.forEach((param) => {
					injectors.push($injector.get(param));
				});
				let props = callbackFunc.apply(null, injectors);
				let module;
					
				if(isExtension) {
					name = name + '.' + $util.guid();
					module = $modules.prepareModule(name, configs.group, args, true);
				}
				else {
					module = $modules.get(name);
				}
					
				module.props = Object.assign({
					toolbar: {
						left: [],
						middle: [],
						right: []
					},
					sidebar: {
						tabs: [],
						icons: []
					}
				}, module.props, props);
				
				await $modules.config(module);
				
				// Initialize extensions..
				modulesConfigured.push(name);
				configs.extensions.forEach(async function(extension) {
					await Zemit.module.apply(null, extension);
				});
			};
			
			// Initialize previously added modules
			if(moduleList.length > 0) {
				
				// Check if there's any module extension..
				moduleList.forEach(module => {
					let extendedModules = extendModuleList.filter(moduleExtension => {
						return moduleExtension[0] === module[0];
					}) || [];
					
					// Apply module dependencies
					extendedModules.forEach(moduleExtension => {
						module[1].dependencies = module[1].dependencies.concat(moduleExtension[1].dependencies);
					});
					
					module[1].extensions = extendedModules;
				});
				
				// Prepare module for initialization..
				moduleList.forEach(module => {
					$modules.prepareModule(module[0], module[1].group, module[1]);
				});
				
				// Adjust module priorities..
				moduleList.forEach(module => {
					let props = module[1];
					module[1].priority = $modules.getLowestPriority(props.priority, props.dependencies);
				});
				moduleList.sort((a, b) => {
					return a[1].priority - b[1].priority;
				});
				
				// Topological sorting
				// TOOD: Take priority into account..
				let graph = {};
				moduleList.forEach(module => {
					graph[module[0]] = module[1].dependencies;
				});
				let sortedModuleNames = $util.topologicalSorting(graph);
				
				// Reorganize module list
				let sortedModules = [];
				sortedModuleNames.forEach(moduleName => {
					let module = moduleList.find(module => {
						return module[0] === moduleName;
					});
					sortedModules.push(module);
				});
				
				for(let i = 0; i < sortedModules.length; i++) {
					let module = sortedModules[i];
					let dependencies = module[1].dependencies;
					for(let i = 0; i < dependencies.length; i++) {
						if(!$modules.isActivated(dependencies[i])) {
							return;
						}
					}
					
					await Zemit.module.apply(null, module);
				}
			}
			
			$hook.run('onAllModulesRan');
		});
	}])
	
	Zemit.app.factory('$modules', ['$ocLazyLoad', '$session', '$hook', '$i18n', '$util', '$injector', '$rootScope', '$debug', '$sidebar', '$toolbar', '$shortcut', '$routeProvider', function($ocLazyLoad, $session, $hook, $i18n, $util, $injector, $rs, $debug, $sidebar, $toolbar, $shortcut, $routeProvider) {
		
		var settings = $session.get('settings');
		$session.prepare('settings', {
			modules: {}
		});
		
		var factory = {
			
			items: {},
			modulesPaths: {},
			
			bootstrap: (modules = []) => {
				
				if(modules instanceof String) {
					modules = [modules];
				}
				
				let loadCallback = () => {
					
					$hook.add('onBeforeReady', () => {
						$hook.run('onAllModulesLoaded');
					});
					
					if($hook.hasRan('onBeforeReady')) {
						$hook.run('onAllModulesLoaded');
					}
				};
				
				var moduleLoadCount = 0;
				modules.forEach(modulePath => {
					
					let modulePathSplitted = modulePath.split('/');
					let name = modulePathSplitted[modulePathSplitted.length - 1];
					
					factory.modulesPaths[name] = modulePath;
					
					$debug.log('module', 'INIT', name.toUpperCase());
					
					if(Zemit.version === 'dev') {
						
						// Initialize a new module
						Zemit.modules[name] = angular.module('zm' + $util.camelize(name, true), []);
						
						// Lazy-load the newly created module
						$ocLazyLoad.load({
							name: name,
							rerun: true,
							reconfig: true,
							files: [
								modulePath + '/' + name + '.js',
								modulePath + '/' + name + '.css'
							]
						}).then(response => {
							
							moduleLoadCount++;
							
							if(moduleLoadCount === modules.length) {
								loadCallback();
							}
						});
					}
					else {
						
						moduleLoadCount++;
						
						if(moduleLoadCount === modules.length) {
							loadCallback();
						}
					}
				});
				
				if(modules.length === 0) {
					loadCallback();
				}
			},
			
			prepareModule: function(name, group = 'misc', props = {}, isExtension = false) {
				
				let modulePath = factory.modulesPaths[name];
				let module = {
					
					isActivated: function() {
						return this._activated;
					},
					activate: function() {
						this._activated = true;
					},
					deactivate: function() {
						this._activated = false;
					},
					
					name: name,
					path: modulePath,
					priority: props.priority || 0,
					dependencies: props.dependencies || [],
					extensions: props.extensions || [],
					group: group,
					props: props || {},
				}
				
				if(!isExtension) {
					
					let options = {
						modules: {}
					};
					options.modules[name] = {
						activated: false
					};
					$session.prepare('settings', options);
					
					let defaultThumbnail = 'data:image/png;base64,';
					
					let settingsDefault = {};
					settingsDefault[name] = props.defaultSettings;
					$session.prepare('settings', {
						modules: settingsDefault
					});
					
					module._activated = settings.modules[name].activated;
					module.title = $i18n.get('modules.' + name + '.title');
					module.desc = $i18n.get('modules.' +  name + '.desc');
					module.thumbnail = defaultThumbnail;
					module.settings = settings.modules[name];
					
					let thumbnail = modulePath + '/' + name + '.png';
					let image = new Image();
					image.src = thumbnail;
					image.onload = () => {
						module.thumbnail = thumbnail;
						$rs.$digest();
					};
					
					if(!this.items[group]) {
						this.items[group] = {
							name: group,
							title: $i18n.get('core.modules.' + group),
							modules: []
						};
					}
					
					this.items[group].modules.push(module);
				}
				
				return module;
			},
			
			config: async function(module) {
				
				if(module.props.onConfig instanceof Function) {
					module.props.onConfig(module);
				}
				if(module.isActivated()) {
					await this.register(module);
				}
				
				return module;
			},
			
			register: async function(module) {
				
				module.activate();
				
				let modulePath = factory.modulesPaths[module.name];
				
				// Initialize shortcuts
				for(let key in module.props.shortcuts) {
					let shortcut = module.props.shortcuts[key];
					$shortcut.add(
						module.name + '_' + key,
						shortcut.key,
						shortcut.title,
						shortcut.callback
					);
				}
				
				// Adjust directives templateUrls
				if(module.props.directives instanceof Object) {
					for(let directiveName in module.props.directives) {
						let directive = module.props.directives[directiveName];
						if(directive.templateUrl) {
							directive.templateUrl = modulePath + '/' + directive.templateUrl;
						}
					}
				}
				
				// Initialize directives
				for(let directive in module.props.directives) {
					Zemit.app.compileProvider.directive(directive, () => {
						return module.props.directives[directive]
					});
				}
				
				// Adjust routes templateUrls
				if(module.props.routes instanceof Array) {
					module.props.routes.forEach(route => {
						if(route.templateUrl) {
							route.templateUrl = modulePath + '/' + route.templateUrl;
							route.files.forEach((file, index) => {
								route.files[index] = modulePath + '/' + route.files[index];
							});
						}
					});
				}
				
				// Initialize factories
				for(let factory in module.props.factories) {
					Zemit.app.provide.factory(factory, module.props.factories[factory]);
				}
				
				let finalizeCallback = () => {
					
					// Initialize modules
					if(module.props.onInit instanceof Function) {
						module.props.onInit(module);
					}
					
					// Implement sidebar tabs
					if(module.props.sidebar.tabs instanceof Array) {
						module.props.sidebar.tabs.forEach(options => {
							$sidebar.addTab(module.name, options)
						});
					}
					
					// Implement sidebar icons
					if(module.props.sidebar.icons instanceof Array) {
						module.props.sidebar.icons.forEach(options => {
							$sidebar.addIcon(module.name, options)
						});
					}
					
					// Implement toolbar regions
					if(module.props.toolbar instanceof Object) {
						let keys = Object.keys(module.props.toolbar);
						keys.forEach(region => {
							let items = module.props.toolbar[region];
							items.forEach(item => {
								$toolbar.addItem(region, module.name, item);
							});
						});
					}
					
					// Initialize routes
					if(module.props.routes instanceof Array) {
						module.props.routes.forEach(route => {
							$routeProvider.when(route.path, {
								name: route.name,
								templateUrl: route.templateUrl,
								controller: route.controller,
								resolve: {
									lazy: ['$ocLazyLoad', function($ocLazyLoad) {
										return $ocLazyLoad.load(route.files);
									}]
								}
							})
						});
					}
				};
				
				// Add included files
				if(module.props.includes instanceof Array
				&& module.props.includes.length > 0) {
					
					let files = [];
					let modulePath = factory.modulesPaths[module.name];
					module.props.includes.forEach(include => {
						files.push(modulePath + '/' + include);
					});
					
					let response = await new Promise((resolve, reject) => {
					    $ocLazyLoad.load({
							serie: true,
							rerun: true,
							reconfig: true,
							files: files
						}).then(response => {
							finalizeCallback();
							resolve(response);
						});
					});
				}
				else {
					finalizeCallback();
				}
			},
			
			getTotal: function() {
				
				let total = 0;
				let keys = Object.keys(this.items);
				for(let i = 0; i < keys.length; i++) {
					
					let modules = this.items[keys[i]].modules;
					for(let y = 0; y < modules.length; y++) {
						total++;
					}
				}
				
				return total;
			},
			
			get: function(name) {
				
				let keys = Object.keys(this.items);
				for(let i = 0; i < keys.length; i++) {
					
					let modules = this.items[keys[i]].modules;
					for(let y = 0; y < modules.length; y++) {
						if(modules[y].name === name) {
							return modules[y];
						}
					}
				}
				
				return null;
			},
			
			getLowestPriority: function(priority, dependencies) {
				
				for(var i = 0; i < dependencies.length; i++) {
					let module = this.get(dependencies[i]);
					if(module && module.priority >= priority) {
						priority = module.priority + 1;
						
						if(module.dependencies.length > 0) {
							priority = this.getLowestPriority(priority, module.dependencies);
						}
					}
				}
				
				return priority;
			},
			
			dependenciesAllActivated: function(module) {
				
				for(var i = 0; i < module.dependencies.length; i++) {
					if(!this.isActivated(module.dependencies[i])) {
						return false;
					}
				}
				
				return true;
			},
			
			isActivated: function(name) {
				
				let module = this.get(name);
				return module && module.isActivated();
			}
		};
		
		return factory;
	}]);
})();